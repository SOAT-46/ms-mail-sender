...
var errSendMailQueueListener = errors.New("failed to send error mail")
...
func TestSendErrorMailQueueListener(t *testing.T) {
	t.Run("should log success when mail is sent successfully", func(t *testing.T) {
		...
		listener := listeners.NewSendErrorMailQueueListener(command)
	...
	t.Run("should log error when mail sending fails", func(t *testing.T) {
		...
		command := doubles.NewInMemorySendMailCommand().WithError(errSendMailQueueListener)
		listener := listeners.NewSendErrorMailQueueListener(command)
		...
		assert.Contains(t, hook.LastEntry().Message, "failed to send error mail", "should log error message")
		assert.Contains(t,
			hook.LastEntry().Message,
			errSendMailQueueListener.Error(),
